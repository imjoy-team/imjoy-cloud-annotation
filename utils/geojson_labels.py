...
# %% Scan folders for geojson and save as png
path_results = Path(r'PASTE-PATH-TO-DATA')  # Folder with segmentation results from imjoy-cloud-annotation
SAVE_ALSO_COLOR_IMAGE = True                # Should a color version of the label images be stored as well (for visualization purposes) 
...
for folder in path_results.glob("*/"):
    ...
    #  Get image size
    annotation_folder = folder / "target_files_v0"
    for json_file in annotation_folder.glob('*.json'):
    ...
    # Save array of label
    for annotation_folder in folder.glob("*target_files_v*"):
        for json_file in annotation_folder.glob('*.json'):
            with open(str(json_file), encoding='utf-8-sig') as fh:
                data_json = json.load(fh)
