...
where `i` is a version number. `v0` is the originally provided target file, e.g. the original segmentation files, subsequent versions, e.g. `v1`, `v2` are created if for this sample a new annotation was submitted. Some important considerations
...
In each `target_files` folder you will find the a **status file** (with its name specified in the Task Dashboard as `Sample Status File`) providing information about the annotation (user, date, ...). This file can thus be used to analyze annotations by different users.